jQuery(document).ready(function($) {
	...
	function CMB2ConditionalsInit(changeContext, conditionContext) {
		...
		changeContext.on('change', 'input, textarea, select', function(evt) {
			...
			if( dependants.length > 0 ) {
				...
				if( elm.attr('type') === 'checkbox' ) {
					var checkedValues = $('[name="'+fieldName+'"]:checked').map( function() {
						return this.value;
					}).get();
				...
				dependants.each( function(i,e) {
					...
					if( typeof currentFieldName !== 'undefined' && currentFieldName !== '' && $.inArray( currentFieldName, dependantsSeen ) < 0 ) {
		...
		conditionContext.on( 'click', '.cmb-multicheck-toggle', function(evt) {
			evt.preventDefault();
			var button = $( this );
			var multiCheck = button.closest( '.cmb-td' ).find( 'input[type=checkbox]:not([disabled])' );
		...
		var requiredElms = $('[data-conditional-id][required]', conditionContext);
